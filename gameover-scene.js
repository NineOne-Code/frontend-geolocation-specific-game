    ...
    create: async function() {
        try {
            if (this.player.username && this.player.score) {
                await fetch("https://backend-nineone-code.herokuapp.com/api/game/geolocation-specific-game/create", {
                    "method": "POST",
                    "headers": {
            ...
            this.globalScores = await fetch("https://backend-nineone-code.herokuapp.com/api/game/geolocation-specific-game/get")
                .then(response => response.json());
            this.nearbyScores = await fetch(`https://backend-nineone-code.herokuapp.com/api/game/geolocation-specific-game/getNearLocation?latitude=${this.player.location.coordinates[1]}&longitude=${this.player.location.coordinates[0]}`)
                .then(response => response.json());
